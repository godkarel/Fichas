...
local function constructNew_frmItemDaLista3()
    ...
    obj.layout1:setMargins({left=1, right=4, top=0, bottom=0});
    ...
    obj.imgIconSkill:setParent(obj.btnAtualSkill);
    obj.imgIconSkill:setName("imgIconSkill");
    obj.imgIconSkill:setAlign("client");
    ...
    obj.dataLink1:setFields({'DanoAtributo1', 'DanoHabilidade1', 'CuraHabilidade1', 'FixoHabilidade1'});
    obj.dataLink1:setName("dataLink1");

		 					
		local function ExecutarHDaActionbar() 
		
			--[[ HABILIDADE DE DANO ]]--
			-- obter a mesa do personagem

			local node = globalForm.ListaJutsus3.node;

			if 0 == node.RecargaHabilidade then
		
				local mesaDoPersonagem = Firecast.getMesaDe(sheet);
				local mesas = rrpg.getRooms();
				local bibliotecaAtual = mesas[1].library;

					if globalForm.cmbTipoGrupo.value == "1" then 
						local function obterNomesRecursivo(bibItem)
							...
							for i = 1, #itensFilhos, 1 do
								...
								if nomesDoFilho == "Sistema de Combaate Velen" then
									...
									local idPersonagem = globalForm.cmbInimigos.value;
						...
						fieldficha.AcertoMagico = tonumber(fieldficha.AcertoMagico) or 0;						
						mesaDoPersonagem.chat:rolarDados("1d20", "[§K3]Executar Habilidade",
						function (rolado)				
						if node.DanoAtributo1 ~= nil then
							if fieldficha.AcertoMagico + 1 > rolado.resultado then							
								if fieldficha.AcertoMagico >= rolado.resultado then
									...
									local Custo2 = fieldficha.MPAtual;
									node.Custo2 = fieldficha.MPAtual;
									node.Custo2 = (tonumber(node.Custo2) or 0) - (tonumber(node.CustoHabilidade) or 0);
									fieldficha.MPAtual = node.Custo2;
									mesa.meuJogador:requestSetBarValue(2, node.Custo2);
									node.ContaRecarga = node.TurnoAtual	
									node.RecargaHabilidade = tonumber(node.ContaRecarga) + tonumber(node.CDHabilidade) + 1 -  tonumber(node.TurnoAtual)	
									if node.DanoAtributo1 == 'Cura' or node.DanoAtributo1 == 'Cura Fixa' then
										if 1 == rolado.resultado then										
											nodeExterno.AlvoRecebido = globalForm.cmbInimigos.value
											nodeExterno.GrupoRecebido = globalForm.cmbTipoGrupo.value	
										...
										if fieldficha.CMagico +1 > rolado.resultado then 										
											nodeExterno.AlvoRecebido = globalForm.cmbInimigos.value
											nodeExterno.GrupoRecebido = globalForm.cmbTipoGrupo.value
											if node.DanoAtributo1 ~= 'BUFF' then
											...
											if node.DanoAtributo1 == 'REAL PA' or node.DanoAtributo1 == 'REAL PF' or node.DanoAtributo1 == 'REAL PM' then
					...
					if globalForm.cmbTipoGrupo.value == "2" then 
								...
								if nomesDoFilho == "Sistema de Combaate Velen" then
									...
									for _, node in ipairs(nodesO) do
										if node.NomeDoPersonagemVez == idPersonagem then  -- Verifica se o campo NomeDoPersonagemVez existe
											PERAlvo = tonumber(node.Pers or 1)
											RESAlvo = tonumber(node.RES or 1)
										end
									end
								end
							end
    ...
    obj.dataLink2:setField("NomeHabilidade");
    ...
    obj._e_event1 = obj.btnAtualSkill:addEventListener("onClick",
        function (event)
            local node = nodeSelecionado; -- Acessa o nó armazenado.
            
            					if node then
            						if globalForm then
            							globalForm.ListaJutsus3.node = node; -- Acessa `ListaJutsus3` através da referência global.                    
            							globalForm.ListaJutsus3.visible = true;
            
            							node.Hintensidade1 = tonumber(node.Hintensidade1) or 0;
            
            							Dialogs.confirmYesNo(node.NomeHabilidade,
            							function (confirmado)
            								if confirmado then
            									if globalForm.ListaJutsus3.node == nil	then
            										showMessage("Selecione a Habilidade")
            									else
            										ExecutarHDaActionbar();
            									end;
            								else
            								end
            							end);
            						else
            							showMessage("O formulário principal não está disponível!");
            						end
            					else
            						showMessage("Nenhuma habilidade foi selecionada!");
            					end
        end);
        ...
        function (field, oldValue, newValue)
            if FichaPrincipal ~= nil then               
                                  	if sheet.DanoAtributo1 == 'PA' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber(FichaPrincipal.PA) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;			
            						if sheet.DanoAtributo1 == 'PM' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber(FichaPrincipal.PM) or 0)						
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;
            						if sheet.DanoAtributo1 == 'PF' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber(FichaPrincipal.PF) or 0)	
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;							
            
            						if sheet.DanoAtributo1 == 'Cura' then
            							sheet.Hintensidade1 =(tonumber(sheet.CuraHabilidade1) or 0) + (tonumber(FichaPrincipal.PM) or 0)	
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;
            						if sheet.DanoAtributo1 == 'Cura Fixa' then
            							sheet.Hintensidade1 =(tonumber(sheet.CuraHabilidade1) or 0);	
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;
            
            						if sheet.DanoAtributo1 == 'PA+M/PA' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber(FichaPrincipal.PA * 1.5) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;	
            						if sheet.DanoAtributo1 == 'PF+M/PF' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber(FichaPrincipal.PF * 1.5) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;	
            						if sheet.DanoAtributo1 == 'PM+M/PM' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber(FichaPrincipal.PM * 1.5) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;	
            
            
            						if sheet.DanoAtributo1 == 'M/PA' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber(FichaPrincipal.PA / 2) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;	
            						if sheet.DanoAtributo1 == 'M/PF' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber(FichaPrincipal.PF / 2) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;	
            						if sheet.DanoAtributo1 == 'M/PM' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber(FichaPrincipal.PM / 2) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;	
            
            						if sheet.DanoAtributo1 == 'PA+PF' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber(FichaPrincipal.PA + FichaPrincipal.PF) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;	
            						if sheet.DanoAtributo1 == 'PF+PM' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber(FichaPrincipal.PF + FichaPrincipal.PM) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;
            						if sheet.DanoAtributo1 == 'PA+PM' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber(FichaPrincipal.PA + FichaPrincipal.PM) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;
            
            						if sheet.DanoAtributo1 == 'M/PA+PF' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber((FichaPrincipal.PA /2) + FichaPrincipal.PF) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;
            						if sheet.DanoAtributo1 == 'M/PA+PM' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber((FichaPrincipal.PA /2) + FichaPrincipal.PM) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;
            						if sheet.DanoAtributo1 == 'M/PF+PM' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber((FichaPrincipal.PF /2) + FichaPrincipal.PM) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;
            						if sheet.DanoAtributo1 == 'M/PF+PA' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber((FichaPrincipal.PF /2) + FichaPrincipal.PA) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;
            						if sheet.DanoAtributo1 == 'M/PM+PF' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber((FichaPrincipal.PM /2) + FichaPrincipal.PF) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;
            						if sheet.DanoAtributo1 == 'M/PM+PA' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber((FichaPrincipal.PM /2) + FichaPrincipal.PA) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;
            
            						if sheet.DanoAtributo1 == 'M/PA+M/PF' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber((FichaPrincipal.PA /2) + FichaPrincipal.PF / 2) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;
            						if sheet.DanoAtributo1 == 'M/PF+M/PM' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber((FichaPrincipal.PF /2) + FichaPrincipal.PM / 2) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;
            						if sheet.DanoAtributo1 == 'M/PA+M/PM' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0) + (tonumber(sheet.CuraHabilidade1) or 0) + (tonumber((FichaPrincipal.PA /2) + FichaPrincipal.PM / 2) or 0)								
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;
            
            						if sheet.DanoAtributo1 == 'REAL PA' or sheet.DanoAtributo1 == 'REAL PF' or sheet.DanoAtributo1 == 'REAL PM' then
            							sheet.Hintensidade1 = (tonumber(sheet.FixoHabilidade1) or 0)	
            							sheet.naoEBuff = true;
            							sheet.verificaBuff = sheet.naoEBuff;
            						end;
            
            						if sheet.DanoAtributo1 == 'Efeito' then
            							sheet.Hintensidade1 = 0
            						end;
            
            						if sheet.DanoAtributo1 == 'Buff' then
            							sheet.Hintensidade1 = (tonumber(sheet.DanoHabilidade1) or 0)	
            						end;
            					end;
        end);
    ...
    obj._e_event3 = obj.dataLink2:addEventListener("onChange",
        function (field, oldValue, newValue)
            self.imgIconSkill.width="32"
            			...
            			if sheet.NomeHabilidade == "Si" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/df2xhdg-060557f2-5f2f-4139-9c36-57f56b2662c7.png/v1/fill/w_32,h_32,q_80,strp/si_by_karelsama_df2xhdg-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..xe3O2_M3pY-6Z552KnrYJz2lq598t8bc4RzfjYTWlSg"
            				self.imgIconSkill.width = "45"
            				self.imgIconSkill.height ="45"
            			end;
            			...
            			if sheet.NomeHabilidade == "Reanimar" then
            			...
            			if sheet.NomeHabilidade == "Estilhaços de Alma" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/df2zz60-b04b91ab-5620-44f4-803b-3567b7bd8a4a.png/v1/fill/w_32,h_32,q_80,strp/estilhao_de_arma_by_karelsama_df2zz60-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..lLlb21CabKcM2rdhqJLzuPZvcX0tUrIWfvuBA4yKThE"
            			end;
            			...
            			if sheet.NomeHabilidade == "Alvoredo" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkx7kw-53f86089-5fa0-4a89-9bab-6a7dd01830ae.png/v1/fill/w_64,h_64,q_80,strp/alvoredo_by_karelsama_djkx7kw-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..01UD7CXT1mLrbVmrwytHV_BRhLjD7OiHw2jvdvsSYCo"
            			end;
            
            			if sheet.NomeHabilidade == "Transformação do Urso Corjua" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djw57fv-15392f2e-277c-41e8-be8b-b78114162c5c.png/v1/fill/w_372,h_352,q_80,strp/urso_coruja_by_karelsama_djw57fv-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..FbvyzQIk2ly8swNohXtP3YG-rLKTyRtUtKM6lEWWupY"
            			end;
            
            			if sheet.NomeHabilidade == "Transformação Besta Hibrida" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djw5c19-ceb9eab0-7e4f-46ba-b9e4-0be5aa208eb0.png/v1/fill/w_64,h_64,q_80,strp/spell_holy_blessingofstamina_by_karelsama_djw5c19-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..yYZ5Ap6TgPJPV3Xrv-p52VW4JxWhdyWM4ZICzdayuWk"
            			end;
            
            			if sheet.NomeHabilidade == "Fonte da Vida" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkx7ql-2c2e392d-301f-405b-a08d-ee49cfc636e0.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..Cixf1p4H-1f5DngKQTRQerXfkJCI_obq4Y-UkvuB-vo"
            			end;
            
            			if sheet.NomeHabilidade == "Mastigar[ Urso ]" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk3565d-df7c63c2-df7c-4fae-9ee0-4d40c7b4da71.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..q5vXKbOcEC9i0-h0V7S1KkUrWzZbjXmcHkMRDhknh3w"
            			end;
            
            			if sheet.NomeHabilidade == "Investida do Urso[ Urso ]" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk3565t-a4bb5e14-f2e4-42f9-bdfe-d563e1ca7d42.png/v1/fill/w_64,h_64,q_80,strp/carreira_druida_by_karelsama_dk3565t-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..TvFWnLTOg0okWkbowZP-IdEcrXzS67FZPGtqTsKotDc"
            			end;
            
            			if sheet.NomeHabilidade == "Olhar Felino[ Pantera ]" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk35654-e57687e9-3100-45d8-9297-cdc9619d1bd2.png/v1/fill/w_64,h_64,q_80,strp/olhar_felino_by_karelsama_dk35654-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..Yhuf59IZPtYTYHLbOPJkmF43wdA-TlpMmwtxttohKeM"
            			end;
            
            			if sheet.NomeHabilidade == "Instinto Felinmo[ Pantera ]" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk3565o-624c83c4-ce74-42ee-898c-ae780180ab09.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..qb1hDBkTmQryYau1HHumrKITQPrMv3hWqa6j_40hw-Y"
            			end;
            
            
            			if sheet.NomeHabilidade == "Pulso Estelar" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djw5al9-e41021e3-b841-41ad-90d5-e942a572b06e.png/v1/fill/w_64,h_64,q_80,strp/pulso_estelar_by_karelsama_djw5al9-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..OmGB8UmEUSgmY-aYpDGXJYLEn7y3Fdjbi3MVng9xg8w"
            			end;
            
            			if sheet.NomeHabilidade == "Garras Selvagens" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djw5b7y-5efc1a4a-1d04-46a7-b879-3e8d4cee3253.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..rwnDkczOyuhE6ka0hoIawNzLZnohJva_IfhvcEes60E"
            			end;
            			...
            			if sheet.NomeHabilidade == "Golpe Fisico Eletrico" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk35ds6-9c56feef-58fb-4236-9e47-4fbcd2590a9f.png/v1/fill/w_64,h_64,q_80,strp/golpe_fisico_eletrico_by_karelsama_dk35ds6-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..2adFAqlQf8fWsrA8TYYxqUvI15fW1cP_XWmFvCJz-vw"
            			end;
            
            			if sheet.NomeHabilidade == "Marretada de Lava" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk35ds0-0a31bc4f-4aeb-4886-99a3-e427183a9db9.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..U5UAhK0wrHdePiputVowTiZ7rtyOaFlN2RnaiXLePSM"
            			end;
            			...
            			if sheet.NomeHabilidade == "Chuva Vital" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxkh1-2b9a040e-7f7e-41de-b1cb-8e6da4d55508.png/v1/fill/w_64,h_64,q_80,strp/chuva_vital_by_karelsama_djkxkh1-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..4bZqcZtxctfwouYAxa0nEttABosE7Do94JZy1NAkXvE"
            			end;
            
            			if sheet.NomeHabilidade == "Sapo" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxkib-50bc6d43-0592-4edb-8e6f-73416ddaa61b.png/v1/fill/w_64,h_64,q_80,strp/sapo_by_karelsama_djkxkib-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..AaDNgYU0GReUS64K_C3delGl9giMVz1jTi4WG1WE1Ug"
            			end;
            			
            			if sheet.NomeHabilidade == "Espírito do Lobo" then
            			...
            			if sheet.NomeHabilidade == "Link Espiritual" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djnmxv7-c834df50-dc95-4890-8093-cee987e86b8c.png/v1/fill/w_64,h_64,q_80,strp/spell_shaman_spiritlink_by_karelsama_djnmxv7-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..TV4y6AI0y_UZCX5zjMBy9uaV1mN55navPNvyGaPck6c"
            				self.imgIconSkill.width = "45"
            				self.imgIconSkill.height ="45"
            			end;
            			...
            			if sheet.NomeHabilidade == "Ward Sentinela" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkwylw-0b140027-fd31-4930-8ab4-1aee797816ad.png/v1/fit/w_150,h_150,q_70,strp/ward_espia_by_karelsama_djkwylw-150.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..DK7DfPv-IbT1X6l_jE2Y1aWF5uRbCzgnuJPJWgEYBTY"
            			end;
            
            			if sheet.NomeHabilidade == "Flash Aliado" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkwz2j-998baa85-bfe5-468d-ae13-d7903670ce1e.png/v1/fill/w_64,h_64/flash_aliado_by_karelsama_djkwz2j-fullview.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..AeGH-8nPzhtR5ToNmeqo5dGJRHPkij7wGZYpLv-isz4"
            			end;
            			...
            			if sheet.NomeHabilidade == "Flecha Congelante" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxjda-b3e147f6-c69b-4e0a-8495-875cb47d3eaf.png/v1/fill/w_256,h_256,q_80,strp/flecha_congelante_by_karelsama_djkxjda-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..y__vr4gIzUryHuJcNxxR-pUFm7VGnP_5P_8bP8v41rU"
            			end;
            
            			if sheet.NomeHabilidade == "Flecha Concentrada" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxjoc-009aeb52-0ebd-4eed-ac53-de26a0f75351.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..M3Sqg9trjrgeaNkNWpIWsx6K7bHjrt5Ak3oWZqIsrBU"
            			end;
            			...
            			if sheet.NomeHabilidade == "Aproveitando a Cura" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkx6m3-8e0b7582-e1aa-477b-8e1f-22acf179bffd.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..G1ZqMwY3ek9qZs4vedTTLOZzdhE3dSbUQu48moyOcPc"
            			end;
            
            			if sheet.NomeHabilidade == "Carga Sonora" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkx81o-24fe98cf-f89f-4780-bf5b-2f46b4256884.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..zAhuubkiaZCQjylnK0v-5VnZSJBW4dNAhfS477WzMbY"
            			end;
            
            			if sheet.NomeHabilidade == "Lamina Sonora" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djmqsek-f6518e8a-f0eb-43b7-8c70-4b1d0b75c3f1.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..YCIaEz85aytiIf1ZplX7pkohDrWqmsRy6mChHd1_CuU"
            			end;
            			
            			if sheet.NomeHabilidade == "Dó Maior" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djrjnza-63046cb3-b096-4c59-a396-f5543a20dd3d.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..GoCAbrYtzJrvPX7uyK2qD6ZQiwE-M_WCzqG4YfQ1o8c"
            			end;
            
            			if sheet.NomeHabilidade == "Sinfonia Curativa" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkx87r-c049a55a-61f8-469f-b284-a1e9824022bc.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..AzZsOKFOCr6kkHl0O3Soh3rbu1FXdpohu51AfxTIXAg"
            			end;
            			...
            			if sheet.NomeHabilidade == "Diade" then
            			...
            			if sheet.NomeHabilidade == "Flecha Sonora" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxhct-f9de268b-956a-4a78-9cd0-0f48269b7c68.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..LGo6PGmKZOPy02pky8yRQILi7OQipFO-4GV5ebiDIZk"
            			end;
            
            			if sheet.NomeHabilidade == "Notas de Precisão" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxheh-9c1cbe1c-8167-4b3f-9227-722f1fde54e1.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..izLsiaLDIXCYfH1Nox8KvNLmYkzCWZKgzW3Z3oK8g58"
            			end;
            
            			if sheet.NomeHabilidade == "Maestro do Arsenal" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djmp0hl-3371e0d9-b547-4f14-b8eb-137c2f819507.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..hEazUGwIFUHvlNcGKjwj7K8mjDtTR6N0CSSIugYNrnc"
            			end;
            
            			if sheet.NomeHabilidade == "Vinculo Sonoro" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxhfp-04527b4f-7bc1-42b5-bdb4-f50a0c88842a.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..MJT36yCAWyJSmvJROmBTLKWobGJlzceYBZyvfVwBtls"
            			end;
            			
            			if sheet.NomeHabilidade == "Porta do Beneficio Oculto" then
            			...
            			if sheet.NomeHabilidade == "Toque da Necrofago" then
            			...
            			if sheet.NomeHabilidade == "Caixão" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxhv0-81cf3be4-2bdf-44ed-99dd-c1f96d28596e.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..p3NGwrfrRd4fxyUWhF1ZGoJ_ei5A_mjxkOa8MDl9mLw"
            			end;
            
            			if sheet.NomeHabilidade == "Nova Alma" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxhvq-42539c33-cadd-4cf6-a06c-a1e455e0d545.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..LQFNiKqzUoOa9pnkHPHyaASvvuwgY0GSBpC3gLulogM"
            			end;
            			...
            			if sheet.NomeHabilidade == "Controle Mental" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxj02-06c49024-67b1-4e3c-8cd0-b08d4389760e.png/v1/fill/w_64,h_64,q_80,strp/controle_da_mente_by_karelsama_djkxj02-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..hIeHO46x8c9a93APKi0OiIGnaFwaS8gPepzqJsPJWFU"
            			end;
            
            			if sheet.NomeHabilidade == "Feixe de Luz" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxiga-24f0a35a-8486-4e65-a602-2dd2df224ee0.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..T9yn_SRWaVZflcGiVK50oTTfhMNU0zHlibEq0qKLV6c"
            			end;
            			...
            			if sheet.NomeHabilidade == "Lembranças do Oponente" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxgyr-a6f5318d-9db6-4a5d-a346-cea523f2a3fe.png/v1/fill/w_32,h_32,q_80,strp/lembranca_do_oponente_by_karelsama_djkxgyr-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..eabFCYb69umjI5QcNDEVTvszCTbfrkOiCVFNM5yXDuE"
            			end;
            
            			if sheet.NomeHabilidade == "Justiça Veloz" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2oc9l-b40c9d54-5540-481a-b60a-4e49b6836041.png/v1/fill/w_32,h_32,q_80,strp/justia_veloz_by_karelsama_dk2oc9l-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..P-DX2zm525hnxCHo9LivrpNOWxrhAjVgu9VPwUyzDNw"
            			end;
            			if sheet.NomeHabilidade == "Decreto de Morte" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2oc9z-9b5bec8e-b9ab-4b0f-bc95-ef5453cb3f91.png/v1/fill/w_32,h_32,q_80,strp/decreto_de_morte_by_karelsama_dk2oc9z-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..19cav0yoDTB5FGrZ9GvbXAsuUynA6LHIrTAjEwyWggs"
            			end;
            			if sheet.NomeHabilidade == "Protesto" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2oc9b-76a40b79-28ad-4dc8-9fc5-34b76bfd4e0b.png/v1/fill/w_32,h_32,q_80,strp/protesto_by_karelsama_dk2oc9b-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..yDXVZ_df8HzaMGx02tlwd49xgJn5Fd-dVaN9u5Bs7VM"
            			end;
            			if sheet.NomeHabilidade == "Foice Impura" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2oc9t-97d68c11-9f55-47d9-bc75-321d6584a932.png/v1/fill/w_32,h_32,q_80,strp/foice_impura_by_karelsama_dk2oc9t-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..pmCkrxtu5FIsDGcK6TWhZiWE3S8t-qjP81pDrGzaHbw"
            			end;
            			...
            			if sheet.NomeHabilidade == "Golpe Titânico" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkwx7k-4c9bcb95-141d-4f11-8ed9-4a5fa98795f1.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..SR1WPVa-YMYmUT6e1TDoT3DMVXUzKHVY0OjEOkOWvdY"
            			end;
            			...
            			if sheet.NomeHabilidade == "Reflexo Solar" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxl9h-706bb9e0-e053-4bca-b237-8cf102d3c6ee.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..NamGdfJIGTvoX5UQ0_LJQnk4OiOSdnxFyeFAUvJ-RY4"
            			end;
            
            			if sheet.NomeHabilidade == "Alvo de Cura" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxl7p-46c5ffd8-b0d8-47f4-8898-8344fdd0f78e.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..ArspbKpWywdRL5IsEHrCHDDWCOq7z2cNstkDrDDMYGQ"
            			end;
            
            			if sheet.NomeHabilidade == "Reflexo Solar" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2wiok-d028397a-45d0-4a34-b412-84d1caa79ace.png/v1/fill/w_32,h_32,q_80,strp/reflexo_solar_by_karelsama_dk2wiok-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..gX0tiu4qttflum1VJwlCjl6JaSsepG46gUxoKSD9-EA"
            			end;
            			...
            			if sheet.NomeHabilidade == "Penetrar" then
            			...
            			if sheet.NomeHabilidade == "Chute na Garganta" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkwuni-036bbc94-1cfc-4a67-8651-2b2321059c1e.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..R60dfRU_omJXrShulbusYY9iAeylYe_9d2Y8jKY4c0A"
            			end;
            			...
            			if sheet.NomeHabilidade == "Energia Infinita" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkwv92-410aa9c8-4e1d-46bb-b7f5-8d0ac8f870bf.png/v1/fill/w_64,h_64/energia_infinita_by_karelsama_djkwv92-fullview.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..FoiOqvmAkhu-TC5lHF7TpZL6gZgo_JCmOKhtxMBJ4Io"
            			end;
            			...
            			if sheet.NomeHabilidade == "Piscar" then
            			...
            			if sheet.NomeHabilidade == "Drenar Mana" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/df3cbtz-e371ec22-7ca2-4409-9e4b-42bb173e280c.png/v1/fill/w_128,h_128,q_80,strp/drenar_mana_by_karelsama_df3cbtz-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..gMAzAan9Y4mKkuJOLYKqBVSq9y8RROFc8tv8JAkDm1w"
            			end;
            			...
            			if sheet.NomeHabilidade == "Troca Equivalente" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/df3cbtj-6733c0c7-9e33-49c0-8346-7acf37abf325.png/v1/fill/w_32,h_32,q_80,strp/troca_equivalente_by_karelsama_df3cbtj-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..fHgrhXNhEZa5BMm5cUEXEDWkvwQy2VTmS78aCRl96U4"
            			end;
            			...
            			if sheet.NomeHabilidade == "Retirar Mana do Ambiente" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxkvf-d522b0eb-aaac-4407-a802-0b3de8ffd984.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..pi5f51e16Bv-KYfe3-rqLGtqg4-bmuv1Pf988zCtJfM"
            			end;
            
            			if sheet.NomeHabilidade == "Heroico" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxku2-aed831eb-c342-4149-81a2-2169e6120fb2.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..xm0HrjE8ent-yt3v_4k6DHkievaNCl7N_1HUnIf93Jw"
            			end;
            			...
            			if sheet.NomeHabilidade == "Desafiar" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djmp0rd-5073a8de-b53b-46db-9dbe-cdf7c021434e.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..NcQNskiNR36WdB7OPPOh84nm8jz5TSvqAdsazPeimAU"
            			end;
            
            			if sheet.NomeHabilidade == "Sobreviverei" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djmqcyb-00622f26-a310-4510-ab88-57ced170c0c8.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..B-8EXO5zyAhbiJrJjjUDFEW2gxgU9_B0D_nyQ6hxO6Q"
            			end;
            			...
            			if sheet.NomeHabilidade == "Inspiração dos Mares" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxi64-40f91f1c-86b4-4df0-9c3e-65a2d8e3d69c.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..lr7-kSHOaaxqKanu18B7RtgBlq4cKq_3JT6DcJAV6vA"
            			end;
            
            			if sheet.NomeHabilidade == "Livrar-se" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djta46q-25dde1e3-61c7-4d4c-a19c-f4d3fc68f44f.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..fVXXSqcCCkvWMnLOatZwksNHFHImUhacACjsa1-b_ag"
            			end;
            
            			if sheet.NomeHabilidade == "Embebedar o Alvo" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djta7l6-80f901aa-2b13-4fad-a8c9-94404f9705e8.png/v1/fit/w_827,h_687,q_70,strp/embebedar_inimigo_by_karelsama_djta7l6-414w-2x.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..2dwgWYWQ3Zf_9zUT7rebk8Czl9J2jQcKnY_QU11brvY"
            			end;
            
            			if sheet.NomeHabilidade == "Mais uma Espadada" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djta89u-8421bd52-fdec-4ef6-99dc-d2eb8f061236.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..cGGD5wNuWdtnwPDuE4sVGKmlzNmBRuWubFYITDmBEoo"
            			end;
            			...
            			if sheet.NomeHabilidade == "Infundir a Raiva" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2ws58-c4f4b635-bcea-4b4c-ac7f-bb3f6851df23.png/v1/fill/w_32,h_32,q_80,strp/infundir_raiva_by_karelsama_dk2ws58-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..71NZOkv-S6w9TPVrJ-RAoGGO4125k7thEj28iASo5Ic"
            			end;
            			...
            			if sheet.NomeHabilidade == "Bênção do Retorno" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkx9eq-1c36595f-b448-48e0-93a2-338bb9567331.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..MwzVBUzmXFsrTMY2AkXeYPs4MDUHvzg0AJ9_Ft5jOJI"
            			end;
            
            			if sheet.NomeHabilidade == "Meditação Espiritual" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2wsdo-d6b5d425-e52f-42e6-bff4-27e248bf720f.png/v1/fill/w_164,h_180,q_80,strp/meditao_sei_la_by_karelsama_dk2wsdo-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..Mro9zKAqo-yd6OIq0evXNT5JqKth4uhp-fmJVdEaLlA"
            			end;
            
            			if sheet.NomeHabilidade == "Namaste" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2wrwe-e97af6f6-cef7-4cda-9c00-628d03a8d6fb.png/v1/fill/w_32,h_32,q_80,strp/namaste_by_karelsama_dk2wrwe-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..uD-0PUJ9s-5V3wHQCMbn3UYcR6F6M6nj8qqaU5MXruU"
            			end;
            
            			if sheet.NomeHabilidade == "Impacto Ascendente" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxe9n-01b9f4b5-0a9a-47ca-a46b-66ac0049c8e9.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..9ekgovnXsQVsjBW2B5Od33pV3eODkVsO81NMeRr2xrc"
            			end;
            
            			if sheet.NomeHabilidade == "Força é Mana" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2wqx2-9897fc0f-7e39-4c04-8ecc-7a87b376acc7.png/v1/fill/w_32,h_32,q_80,strp/fora__mana_by_karelsama_dk2wqx2-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..H92u6GqVm_isM8WQ1sQ6qStxMKl5k2D1zSqz6D2Uvfo"
            			end;
            
            			if sheet.NomeHabilidade == "Mandamento da Harmonia" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxfbf-9254f6ef-1c3a-4b3e-aebf-896622b66a97.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..S-9hySGlWiBgId3A3DLeP-selYxi5hP4tbW7xINomL8"
            			end;
            			...
            			if sheet.NomeHabilidade == "Bloqueio de Chi" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxg19-f01c4a7e-8327-47a1-ba72-40f31f6deb98.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..BLbxJH8MPShP6eWs3SDwRm8IAeW9bCNeovjXwm9uKe0"
            			end;
            
            			if sheet.NomeHabilidade == "Braços Espirituais" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxg6l-6d182fa9-473d-4f65-8306-3310f0cb8e1c.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..BUWDjTSFxSSOOE76-Yr9KqNNLQI6ViGy8O4ETECh-Sg"
            			end;
            
            			if sheet.NomeHabilidade == "Petalas de Lotus" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxgpy-e54858db-eb57-4f79-8df0-2182a7533769.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..CTwrLeW-ngbVgvleBG0zcCQxEoUDA8UTe8mOvvofXv0"
            			end;
            			
            			if sheet.NomeHabilidade == "Super Força" then
            			...
            			if sheet.NomeHabilidade == "Alma Sanguinária" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkwzth-e50ed5a4-99db-42e2-a48e-1bd96b1632d9.png/v1/fill/w_64,h_64/alma_sanquinaria1_by_karelsama_djkwzth-fullview.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..7qXSY_awR576Np7kcLrPNBh-cQz8jznuLPmbcmrielk"
            			end;
            
            			if sheet.NomeHabilidade == "Pela Escoria" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkx1ns-7434f342-6b4c-4d9b-b9ca-b7936879c9ce.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..v-8foYnNT8Yms-Cz_93Lf59zOZnBMiABRsgiwvZm9cI"
            			end;
            
            			if sheet.NomeHabilidade == "Profanar Alma" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkx21f-8e27b139-2cc7-49fe-a607-d62c693cae26.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..yGm7yIuIAGagrgjzNely-cY-kptZEPuYG1_3kFoF3VY"
            			end;
            			...
            			if sheet.NomeHabilidade == "Asas de Anjo" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxm1t-ce6854be-046a-45b9-a48a-a77c2899d413.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..U5e1jpQ1swBxkD8wJwvBpDQLjh1yNL1cV43x15iypUg"
            			end;
            
            			if sheet.NomeHabilidade == "Critical Cura" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxlpi-3ed75f78-fb18-4435-8ef5-c087c206787c.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..2fetVzKYHMDMWdQbK6D24foV3yJpxBQrIrhXz4jalRs"
            			end;
            
            			if sheet.NomeHabilidade == "Fenda Divina" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxlqf-7f7ddeaf-856b-4dbb-a2fc-f6c47bb1616c.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..spcrqrjFfoAycmv7jzWXPXFk_xW_Bob8-LpR2rdXYiE"
            			end;
            
            			if sheet.NomeHabilidade == "Luz Solar" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/djkxlr6-3a5ef1e4-7b11-42e5-ac6e-6ed96210917c.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..dcT3gEzK6ZhTcjYKnRUl_foprKYyhidO881MVoQ8UM8"
            			end;
            
            			if sheet.NomeHabilidade == "Escudo Refletor" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/df3j326-9f333426-2bd2-44fc-a4b7-5dade17f362f.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..MqMkoMTvCVr-jasepKJf4eS8n9fs9SyG4eUTqtxAWdo"
            			end;
            			...
            			if sheet.NomeHabilidade == "Orbes Entrópicos" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2olvf-837921b6-4f10-415b-9c37-496bc0d577ee.png/v1/fill/w_64,h_64,q_80,strp/orbes_entrpicos_by_karelsama_dk2olvf-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..zjQQig6xLAVu2kXTVsVZx5jAvqafk6jwIcnS-ZYri4E"
            			end;
            
            			if sheet.NomeHabilidade == "Impacto do Caos" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2olvf-837921b6-4f10-415b-9c37-496bc0d577ee.png/v1/fill/w_64,h_64,q_80,strp/orbes_entrpicos_by_karelsama_dk2olvf-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..zjQQig6xLAVu2kXTVsVZx5jAvqafk6jwIcnS-ZYri4E"
            			end;
            
            			if sheet.NomeHabilidade == "Vislumbre do Vazio" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2oiun-0826d9d6-0060-4eb0-8fb1-b6b2ae3d2d42.png/v1/fill/w_32,h_32,q_80,strp/vislumbre_do_vazio_by_karelsama_dk2oiun-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..xn5UqRqOe4Gemv6XWG7R1xdwi9A7iP47k1AjMWhcuhw"
            			end;
            
            			if sheet.NomeHabilidade == "Explosão da Entropia" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2oiuf-08d32849-838f-435d-9e23-fe8fa309b05c.png/v1/fill/w_128,h_128,q_80,strp/exploso_entropica_by_karelsama_dk2oiuf-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..JvuZay8dj6tr7txlZxu3bOz1EaofSiqZkF_NJ-Imerw"
            			end;
            
            			if sheet.NomeHabilidade == "Canalizar o Vazio" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2onmf-4feafc06-6aa8-4c2d-bd62-bea101c4040c.png/v1/fill/w_64,h_64,q_80,strp/canalizar_vazio_by_karelsama_dk2onmf-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..U-GltupBgjmuWI5tyHhzIkzCo1sHoGPPZJidC1LUotw"
            			end;
            
            			if sheet.NomeHabilidade == "Distorção da Sorte" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2oiu3-cf3cba2f-9611-46cc-aee5-aed50d15c566.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..vyJ4XEGWm4gtBvBtUgCCBCM4rrA5T0y3msnM5ic4Rcw"
            			end;
            
            			if sheet.NomeHabilidade == "Buraco Negro" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2omi8-79785065-a3e7-4fef-9454-ed77d8abc07e.png/v1/fill/w_64,h_64,q_80,strp/buraco_negro_by_karelsama_dk2omi8-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..kFtnDfCVJtGZ_hucYa-y4XmZUUze-IgnaIMOMnnlWMc"
            			end;
            
            			if sheet.NomeHabilidade == "Cometa do Caos" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2omm3-537c93bb-fbf8-4fe7-ac65-faf41828f037.png/v1/fill/w_64,h_64,q_80,strp/cometa_do_caos_by_karelsama_dk2omm3-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..qhVIvPAfbcvJljX631gXojgDmCbGXRr7NesxooDUI5k"
            			end;
            
            			if sheet.NomeHabilidade == "Comsumir a Realidade" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2omt3-a868038b-5c37-40d0-aabc-e37185e30882.png/v1/fill/w_64,h_64,q_80,strp/comsumir_a_realidade_by_karelsama_dk2omt3-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..ADhEWjUHl9VrZvbObCST6TJ0S0HCB9w1I-H7bMZFc_o"
            			end;
            
            			if sheet.NomeHabilidade == "Alimentando-se do Caos" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2onju-feb5d14d-8aa9-4939-9af5-6b6e5b9a7ed6.png/v1/fill/w_64,h_64,q_80,strp/alimentando_se_do_caos_by_karelsama_dk2onju-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..E_mCuMYAD2Nds6fKPteLjqt-6RTr0YN0KjCWSjT9maQ"
            			end;
            
            			if sheet.NomeHabilidade == "Companheiro do Vazio" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2om6z-2586c65b-7734-4607-ba60-a7734acdc95c.png/v1/fill/w_64,h_64,q_80,strp/companheiro_do_vazio_by_karelsama_dk2om6z-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..NNg888Zj5M4U43dX783i4IJ6wDEvihuNiaSMVqglDkQ"
            			end;
            
            			if sheet.NomeHabilidade == "Consumo Realidade" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2omt3-a868038b-5c37-40d0-aabc-e37185e30882.png/v1/fill/w_64,h_64,q_80,strp/comsumir_a_realidade_by_karelsama_dk2omt3-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..ADhEWjUHl9VrZvbObCST6TJ0S0HCB9w1I-H7bMZFc_o"
            			end;
            
            			if sheet.NomeHabilidade == "Dimensão Zero" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2oiu0-3172228f-7f9c-493d-a2ba-c292e6564035.png/v1/fill/w_32,h_32,q_80,strp/dimenso_zero_by_karelsama_dk2oiu0-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..eoQmCA1JFOxrzBBc14KDGaeVyqwBCu9qNzIpNg3HG9g"
            			end;
            
            			if sheet.NomeHabilidade == "Espada Entrópica" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2oiua-1f810634-c131-428e-9e74-4a467f0febd1.png/v1/fill/w_128,h_128,q_80,strp/espada_entropica_by_karelsama_dk2oiua-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..8sEN1I87hf7JWixEXS1BtqwT5Q6ztcnwRmxcg1Gr9Ck"
            			end;
            
            			if sheet.NomeHabilidade == "Retirar Espirito do Vazio" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2on9k-f49051df-fdeb-49b7-bc5d-7397c8cfb983.png/v1/fill/w_64,h_64,q_80,strp/retirar_espirito_do_vazio_by_karelsama_dk2on9k-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..RBNn3M5pCFLw-VKvWQVh_rtXZU5X9QL6Bxi0DndvmVA"
            			end;
            
            			if sheet.NomeHabilidade == "Rajada de Orbes Entrópicas" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2olvi-ed4863af-3874-4ff2-9fd3-c65ee1e2bb68.png/v1/fill/w_64,h_64,q_80,strp/rajada_de_orbes_entrpicas_by_karelsama_dk2olvi-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..QFJprWS3EfWBOQjeaZuW7meqgaMaOAmsrMq9A66pzMI"
            			end;
            			
            			if sheet.NomeHabilidade == "Lamina Lendaria dos Dragões" then
            				sheet.fldImgIconSkill = "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/db1c36a0-78ac-4fb7-98ea-1b0aae86ce2c/dk2whte-9398e221-cff9-45c5-92a9-68b4ac3a2fe8.png/v1/fill/w_207,h_215,q_80,strp/lamina_de_lendaria_dos_drages_by_karelsama_dk2whte-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..RRzd5DSTr0gBJBIc2BoP2sI3K_XO4z56oDP_E3l6JyA"
            			end;
    ...
    function obj:destroy() 
        ...
        if self.btnAtualSkill ~= nil then self.btnAtualSkill:destroy(); self.btnAtualSkill = nil; end;
