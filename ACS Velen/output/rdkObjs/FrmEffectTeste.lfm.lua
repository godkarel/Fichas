...
local function constructNew_FrmEffectTeste()
    ...
    obj.button1:setText("Criar Novo Effeito");
    ...
    obj.rclBuffs:setParent(obj);
    obj.rclBuffs:setName("rclBuffs");
    obj.rclBuffs:setField("listaBuffs");
    obj.rclBuffs:setTemplateForm("frmBuff");
    obj.rclBuffs:setTop(40);
    obj.rclBuffs:setWidth(725);
    obj.rclBuffs:setHeight(50);
    ...
    obj.dscBuffSlot:setParent(obj);
    obj.dscBuffSlot:setName("dscBuffSlot");
    obj.dscBuffSlot:setVisible(false);
    obj.dscBuffSlot:setTop(100);
    obj.dscBuffSlot:setWidth(725);
    obj.dscBuffSlot:setHeight(300);
    obj.dscBuffSlot:setMargins({left=4, right=4, top=2});

    obj.rectangle1 = GUI.fromHandle(_obj_newObject("rectangle"));
    obj.rectangle1:setParent(obj.dscBuffSlot);
    ...
    obj.edit1:setField("NomeDOEfeito");
    ...
    obj._e_event1 = obj.rclBuffs:addEventListener("onSelect",
        function ()
            local node = self.rclBuffs.selectedNode;  
            				self.dscBuffSlot.node = node;                       
            				self.dscBuffSlot.visible = (node ~= nil);
            				RclSelecionadoBuff = sheet.listaBuffs
            				
            
            				if self.dscBuffSlot.visible == true then
            					if self.dscBuffSlot.node.NomeEfeitoO == nil and self.dscBuffSlot.node.NomeEfeitoO == "" then
            						self.dscBuffSlot.node.NomeEfeitoO = "EFEITO";
            					end;
            					...
            					self.dscBuffSlot.node.DuraEffect = tonumber(self.dscBuffSlot.node.DuraEffect) or 9000;
            					...
            					self.dscBuffSlot.node.DanoEffect = tonumber(self.dscBuffSlot.node.DanoEffect) or 0;
            					self.dscBuffSlot.node.CustoEffect = tonumber(self.dscBuffSlot.node.CustoEffect) or 0;
            					self.dscBuffSlot.node.CuraEffect = tonumber(self.dscBuffSlot.node.CuraEffect) or 0;
            					self.dscBuffSlot.node.RegMPEffect = tonumber(self.dscBuffSlot.node.RegMPEffect) or 0;
            
            					if self.dscBuffSlot.node.ExpiraEffect == nil or self.dscBuffSlot.node.ExpiraEffect == "" then
            						self.dscBuffSlot.node.ExpiraEffect = 9999
            					end;
...
function newFrmEffectTeste()
    local retObj = nil;
    __o_rrpgObjs.beginObjectsLoading();

    __o_Utils.tryFinally(
      function()
        retObj = constructNew_FrmEffectTeste();
      end,
      function()
...
local _FrmEffectTeste = {
    newEditor = newFrmEffectTeste, 
    new = newFrmEffectTeste, 
    name = "FrmEffectTeste", 
    dataType = "", 
    formType = "undefined", 
...
FrmEffectTeste = _FrmEffectTeste;
Firecast.registrarForm(_FrmEffectTeste);

return _FrmEffectTeste;
